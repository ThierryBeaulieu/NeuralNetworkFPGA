...
for i in range(0, 5000):
    ...
    theta0_UInt8 = np.array([x * 2**weightPrecision0 for x in theta0]).astype(np.int8) # << 2
    ...
    imagePrecision = 2 # 2
    image_UInt8 = np.array([x * 2**imagePrecision for x in xp]).astype(np.int8) # << 2


    ## Step 3. We Make the Dot Product Between Image UInt8 and Weight UInt8
    hiddenLayer0_UInt8 = np.dot(image_UInt8, theta0_UInt8.T) # [4,4]
    ...
    weightPrecision1 = 1
    theta1_UInt8 = np.array([x * 2**weightPrecision1 for x in theta1]).astype(np.int8) # << 2
    ...
    if debug:
        print("FPGA Output:", sig1_UInt8)

    # Calculer dans le notebook
    pred = sig1_UInt8.argmax()
    if pred == y[i]:
        imageMatched += 1
